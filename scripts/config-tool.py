...
def _default_cmd_config(creds) -> str:
    paths = getattr(creds, "paths", None)
    if paths and getattr(paths, "cmd_config", None):
        return str(paths.cmd_config)
    return DEFAULT_REMOTE_PATH


def _plugin_config_path(creds, plugin: str) -> str:
    paths = getattr(creds, "paths", None)
    base = (
...
def main() -> int:
    ...
    # Resolve remote path defaults from login.config [paths]
    if args.plugin:
        args.path = _plugin_config_path(creds, args.plugin)
    elif not args.path or args.path == DEFAULT_REMOTE_PATH:
        args.path = _default_cmd_config(creds)
